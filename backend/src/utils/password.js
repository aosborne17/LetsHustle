...
export class Password {
  static async toHash(password) {
    ...
    const buf = await scryptAsync(password, salt, 64); // simply telling typscript that the 'buf variable is simply a buffer
  ...
  static async compare(storedPassword, suppliedPassword) {
    ...
    const buf = await scryptAsync(suppliedPassword, salt, 64); // simply telling typscript that the 'buf variable is simply a buffer
